...
```
Пользователь → http://18.197.7.122/health
                         │
                    ┌─────▼──────┐
                    │   Nginx    │  :80
                    │  (proxy)   │
...
|--------|----------|----------------|
| `SERVER_HOST` | IP сервера (`18.197.7.122`) | deploy (Ansible inventory) |
...
GitHub Actions (ci-cd.yml)
        ...
        │   [webservers]
        │   production ansible_host=18.197.7.122
...
| Сервис | URL | Логин |
|--------|-----|-------|
| Grafana | http://18.197.7.122:3000 | Из `.env` (`GF_SECURITY_ADMIN_USER` / `GF_SECURITY_ADMIN_PASSWORD`) |
| Prometheus | http://18.197.7.122:9090 | Без авторизации |
| Prometheus Targets | http://18.197.7.122:9090/targets | flask-app должен быть UP |
...
Infrastructure Recovery: IP 18.197.7.122   ← ТЕКУЩИЙ (новый Elastic IP)
EC2 Instance:           i-059c8320d831be2bf
─────────────────────────────────────────────────────
...
│  🏥 Health:      http://18.197.7.122/health                       │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
